...
def calculate_sbert_similarity(master_text, student_text, sbert_model):
    if sbert_model is None:
        print("⚠️ sbert_model is None. Initializing default sbert model.")
...
def calculate_e5_similarity(master_text, student_text, tokenizer, model):
    ...
    if tokenizer is None:
        print("⚠️ tokenizer is None. Initializing default tokenizer.")
        tokenizer = AutoTokenizer.from_pretrained("intfloat/e5-large-v2")
    
    if model is None:
        print("⚠️ model is None. Initializing default model.")
